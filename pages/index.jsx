...
function Page({ children, dir="right", style={} }) {
  ...
  return (
    <div style={{ animation: v ? `${anim} 0.38s cubic-bezier(.22,1,.36,1) both` : "none", minHeight:"100vh", ...style }}>
      {children}
    </div>
  );
}
...
function ScrollCard({ children, delay=0 }) {
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(es => {
      if (es[0].isIntersecting) { el.classList.add('visible'); obs.disconnect(); }
    }, { threshold: 0.1 });
    obs.observe(el);
...
function Bubble({ word, color, size, selected, onClick }) {
  ...
  function handle() {
    if (!selected) { setPopped(true); setTimeout(() => setPopped(false), 500); }
    onClick();
  }
  return (
    <button onClick={handle} style={{
      ...
      border:`2px solid ${color.border}`, borderRadius:9999,
      cursor:"pointer", fontFamily:font, fontSize:fs, fontWeight:700,
      color: selected ? "#fff" : color.text,
      transition:"all 0.2s cubic-bezier(.34,1.56,.64,1)",
      transform: selected ? "scale(0.94)" : popped ? "scale(1.1)" : "scale(1)",
      boxShadow: selected ? `0 6px 24px ${color.glow},0 0 0 4px ${color.border}44` : `0 2px 8px ${color.glow}44`,
...
function LoadingScreen() {
  return (
    <div style={{ maxWidth:390, margin:"0 auto", minHeight:"100vh", background:"linear-gradient(160deg,#1E0A3C,#4C1D95,#6D28D9)", fontFamily:font, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:28, position:"relative", overflow:"hidden" }}>
      ...
      <p style={{ fontSize:18, fontWeight:800, color:"#fff", margin:0 }}>AI가 분석하고 있어요</p>
      <div style={{ display:"flex", gap:10 }}>
        {[0,1,2].map(i => <div key={i} style={{ width:10, height:10, borderRadius:"50%", background:"rgba(196,181,253,0.8)", animation:`bnc 1.3s ${i*0.22}s infinite` }}/>)}
      </div>
    </div>
  );
}
...
function ResultCard({ icon, title, text, gradient, delay=0 }) {
  ...
  return (
    <ScrollCard delay={delay}>
      <div style={{ background:gradient||"#1F2937", borderRadius:24, padding:"18px 20px", marginBottom:12, position:"relative", overflow:"hidden" }}>
...
export default function App() {
  ...
  async function next() {
    ...
    try {
      ...
      // 2차 시도: exclude 없이
      if (ws.length < 2) {
        ws = await fetchWords(word, needed, []);
      }

      if (ws.length >= 2) {
        setWords(ws); setSi(nextSi); setScreen("game");
        // 3단계에서 힌트
        if (nextSi === 2) {
        ...
        setPath(path); setScreen("game");
      }
    } catch {
      setErr("오류가 발생했어요."); setPath(path); setScreen("game");
    }
  }

  function restart() {
    setScreen("home"); setInp(""); setPath([]); setWords([]); setSi(0); setSel(null);
    setAi(null); setFw(null); setErr(""); setHint(null); setSelCat(null); setLetter(null); setEmotion(null);
  }
    ...
    <Page dir="up" style={{ background:"#0F0A1E", fontFamily:font }}>
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:5, marginBottom:20, position:"relative" }}>
          {path.map((w,i) => (
            <span key={i} style={{ display:"flex", alignItems:"center", gap:5 }}>
              ...
              <span style={{ fontSize:12, fontWeight:700, color:"rgba(255,255,255,0.9)", background:"rgba(255,255,255,0.1)", border:"1px solid rgba(255,255,255,0.15)", padding:"3px 10px", borderRadius:20 }}>{w}</span>
            </span>
          ))}
        </div>
        <div style={{ background:"linear-gradient(135deg,rgba(139,92,246,0.3),rgba(109,40,217,0.5))", border:"1px solid rgba(196,181,253,0.4)", borderRadius:24, padding:"20px 22px" }}>
      ...
      <div style={{ padding:"0 20px 40px", background:"#0F0A1E" }}>
          ...
          <ScrollCard delay={0}>
            <div style={{ background:`linear-gradient(135deg,${emotion.color||"#EDE9FE"},${emotion.color||"#EDE9FE"}88)`, borderRadius:24, padding:"18px 20px", marginBottom:12 }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:14 }}>
                ...
                <div>
                  <p style={{ fontSize:17, fontWeight:900, color:"#1F2937", margin:"0 0 4px" }}>{emotion.label}</p>
                  <p style={{ fontSize:13, color:"#374151", margin:0, lineHeight:1.6 }}>{emotion.desc}</p>
                </div>
              </div>
            </div>
          </ScrollCard>
        ...
        <ScrollCard delay={540}>
          <button onClick={restart} style={{ width:"100%", padding:"17px", borderRadius:20, background:"linear-gradient(135deg,#7C3AED,#6D28D9)", color:"#fff", border:"none", fontSize:16, fontWeight:800, cursor:"pointer", fontFamily:font, boxShadow:"0 8px 32px rgba(109,40,217,0.4)", marginTop:4 }}>
            다시 탐색하기 ✨
          </button>
        </ScrollCard>
      </div>
  ...
  if (screen === "game") return (
    <Page dir="right" style={{ background:"#F8F7FF", fontFamily:font, paddingBottom:140 }}>
      <style>{CSS}</style>
      <div style={{ background:"linear-gradient(180deg,#2D1464,#4C1D95,#6D28D9)", padding:"24px 20px 28px", borderRadius:"0 0 32px 32px", boxShadow:"0 8px 32px rgba(109,40,217,0.3)" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:18 }}>
          ...
          <div style={{ display:"flex", gap:5 }}>
            {Array.from({length:MAX}, (_,i) => (
              <div key={i} style={{ height:6, borderRadius:20, transition:"all 0.45s ease", width:i<=si?22:7, background:i<si?"rgba(255,255,255,0.5)":i===si?(isFinal?"#FCD34D":"#fff"):"rgba(255,255,255,0.2)" }}/>
            ))}
          </div>
            ...
            <div style={{ display:"flex", flexWrap:"wrap", gap:4 }}>
              {path.slice(0,-1).map((w,i) => (
                <span key={i} style={{ display:"flex", alignItems:"center", gap:4 }}>
                  {i>0 && <span style={{ color:"rgba(255,255,255,0.35)", fontSize:11 }}>→</span>}
                  <span style={{ fontSize:13, fontWeight:700, color:"rgba(255,255,255,0.95)", background:"rgba(255,255,255,0.12)", border:"1px solid rgba(255,255,255,0.2)", padding:"4px 12px", borderRadius:20 }}>{w}</span>
                </span>
              ))}
            </div>
          </div>
        ...
        <h2 style={{ fontSize:36, fontWeight:900, margin:0, color:"#fff", letterSpacing:"-1px" }}>{path[path.length-1]}</h2>
      </div>
      ...
      {/* 다음 버튼 - 항상 하단 고정 */}
      {sel !== null && (
        <div style={{ position:"fixed", bottom:0, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:390, padding:"16px 20px", background:"rgba(248,247,255,0.95)", backdropFilter:"blur(20px)", borderTop:"1px solid rgba(196,181,253,0.3)", zIndex:100 }}>
    ...
    <Page dir="left" style={{ background:"linear-gradient(160deg,#1E0A3C,#2D1464,#4C1D95)", fontFamily:font, minHeight:"100vh", padding:"48px 20px 80px", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ position:"relative" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:28 }}>
          <h1 style={{ fontSize:28, fontWeight:900, color:"#fff", margin:0 }}>🧠 브레인스토밍</h1>
        ...
        <div onClick={() => startGame("오늘")} style={{ background:"linear-gradient(135deg,rgba(15,23,42,0.8),rgba(30,58,138,0.6))", border:"1px solid rgba(147,197,253,0.2)", borderRadius:20, padding:"14px 18px", marginBottom:14, cursor:"pointer", display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div>
            <p style={{ fontSize:14, fontWeight:800, color:"#fff", margin:0 }}>오늘 하루 마무리</p>
            <p style={{ fontSize:12, color:"rgba(255,255,255,0.5)", margin:0 }}>오늘의 감정을 탐색으로 정리해요</p>
          </div>
